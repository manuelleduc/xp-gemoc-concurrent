...
class ECoreUtil {
	def static Context getContext(Statement _self) {
		...
		val block = (currentStatement as Block)
...
class IntVarAccessAspect extends IntExpressionAspect {
	def int getCurrentValue() {
		val context = ECoreUtil.getContext(_self)
		context.getInt(_self.intVarRef.name)
	}
...
class BoolVarAccessAspect extends BoolExpressionAspect {
	def boolean getCurrentValue() {
		val context = ECoreUtil.getContext(_self)
		context.getBool(_self.boolVarRef.name)
	}
}
